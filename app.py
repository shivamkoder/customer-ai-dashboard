    ...
    if uploaded_files:
        if st.button("🔄 Merge & Load Uploaded Files"):
            try:
                ...
                st.session_state.predicted_cols = []
                st.session_state.feature_importance = None  # Reset
...
if st.session_state.df is not None:
    ...
    # ---------- DISPLAY DASHBOARD & INSIGHTS (ONLY IF PREDICTIONS EXIST) ----------
    if 'Predicted Satisfaction' in st.session_state.df.columns:
        ...
        # ------------------------------------------------------------
        # 🚀 NEW: KEY BUSINESS INSIGHTS SECTION
        # ------------------------------------------------------------
        st.divider()
        ...
        # --- TAB 1: FEATURE IMPORTANCE (Top 5 Drivers) ---
        with tab1:
            if st.session_state.feature_importance:
                # Get top 10 to show, but highlight top 5
                ...
                display_names = []
                for name in names:
                    # Clean up one-hot encoded names (e.g., "City_New York" -> "City: New York")
                ...
                # Textual summary for store owners
                if len(st.session_state.feature_importance) >= 5:
                    ...
                    top5 = st.session_state.feature_importance[4][0]
                    
                    st.success(f"""
                    💡 **Executive Summary**: 
                    1. **{top1}** is the #1 predictor of satisfaction. 
                    2. **{top2}** is the second most important.
                    3. Focus your retention efforts on improving **{top1}** and **{top2}** before sending discounts.
                    """)
            else:
                st.info("Re-run predictions to see the top drivers of satisfaction.")

        # --- TAB 2: CUSTOMER PROFILES (Segment Averages) ---
        with tab2:
            st.subheader("👤 What does the 'Average' Customer Look Like?")
            
            # Group by satisfaction and calculate means
            ...
            # Visual comparison for spending and rating
            fig_profile = px.bar(
                profile_df, 
        ...
        # --- TAB 3: CITY & MEMBERSHIP RISK HEATMAP ---
        with tab3:
            st.subheader("🌍 Which Cities / Memberships are Most At-Risk?")
            
            # Cross-tabulation of City and Membership Type vs Satisfaction
            ...
            # Filter for Unsatisfied to show risk
        ...
        # --- TAB 4: DISCOUNT IMPACT (The Myth Buster) ---
        with tab4:
            st.subheader("📉 Do Discounts Actually Improve Satisfaction?")
            
            # Ensure Discount Applied is numeric for grouping
            df_viz['Discount Applied'] = df_viz['Discount Applied'].astype(int)
            
            # Group by Discount and Satisfaction
        ...
        # --- CONTINUE WITH THE FULL DATAFRAME VIEW (As before) ---
        st.divider()
        ...
        # If loaded but not predicted yet
